...
import styles from './KikBoard.css';

export default class KikBoard extends Component {
    initialState = {   
        currentPlayer : 0,
        value : {
            '00' : '',
    ...
    winnerDivStyles = 'winner' + ',' + 'noDisplay';
    playAgainStyles= 'noDisplay';
    boardItems = [];
    createBoardDivs(){
        this.boardItems = [];
        for(let i=0; i<3; i++){
            for(let j = 0; j<3; j++){
                ...
                this.boardItems.push(<div xaxis={i} yaxis={j} key= {i.toString() + j.toString()} 
                            onClick={this.showValue}>{this.state.value[key]}
                </div>);
            }
        }
    ...
    showValue = (event) => {
        ...
        this.chceckWinner(xAxis, yAxis, newValue[keyToChange], newValue);
    };

chceckWinner(xAxis, yAxis, currentValue, values){
...
resetBoard = () => {
    ...
    this.winnerDivStyles = "winner" + ',' + 'noDisplay';
    this.playAgainStyles = 'noDisplay';
}
styles
render(){
    console.log(styles);
